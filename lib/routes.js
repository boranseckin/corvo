...
FlowRouter.triggers.enter([userCheck], { only: ['hw', 'url'] });
...
FlowRouter.route('/logout', {
    ...
    triggersEnter: [function(context, redirect) {
        Meteor.call('track.newAction', Meteor.userId(), 'user.logout', Meteor.userId());
        Meteor.logout();
        redirect('/');
    }],
});
...
hwRoutes.route('/', {
    ...
    action: () => {
        mount(App, {
            content: <HWTrack classID="home" />,
            redirect: 'false',
        });
    },
});

hwRoutes.route('/:classID', {
    name: 'hw',
    action: (params) => {
        mount(App, {
            content: <HWTrack classID={params.classID} />,
            redirect: 'false',
        });
    },
});
